...
```bash
PHASE_INFO=$(node ~/.claude/get-shit-done/bin/arc-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-shit-done/bin/arc-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-shit-done/bin/arc-tools.js find-phase "${PHASE}" --raw)
```
