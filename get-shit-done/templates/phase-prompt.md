...
> **Note:** Planning methodology is in `agents/arc-planner.md`.
...
**Wave is pre-computed:** Wave numbers are assigned during `/arc:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
