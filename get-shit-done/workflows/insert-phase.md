...
Example: `/arc:insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /arc:insert-phase <after> <description>
Example: /arc:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/arc-tools.js init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node ~/.claude/get-shit-done/bin/arc-tools.js phase insert "${after_phase}" "${description}")
```
...
- Don't use this for planned work at end of milestone (use /arc:add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /arc:plan-phase)
