...
```bash
timestamp=$(node ~/.claude/get-shit-done/bin/arc-tools.js current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
