...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/arc-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="arc-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn arc-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
node ~/.claude/get-shit-done/bin/arc-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/get-shit-done/bin/arc-tools.js state update-progress

# Record execution metrics
node ~/.claude/get-shit-done/bin/arc-tools.js state record-metric \
...
node ~/.claude/get-shit-done/bin/arc-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/get-shit-done/bin/arc-tools.js state add-blocker "Blocker description"
```
</step>
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js state record-session \
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/arc:execute-phase {phase}` + `/arc:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/arc:plan-phase {Z+1}` + `/arc:verify-work {Z}` + `/arc:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/arc:complete-milestone` + `/arc:verify-work` + `/arc:add-phase` |
