...
- Example: `/arc:remove-phase 17` → phase = 17
- Example: `/arc:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /arc:remove-phase <phase-number>
Example: /arc:remove-phase 17
```
...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/arc-tools.js init phase-op "${target}")
```
...
To abandon current work, use /arc:pause-work instead.
```
...
```bash
RESULT=$(node ~/.claude/get-shit-done/bin/arc-tools.js phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node ~/.claude/get-shit-done/bin/arc-tools.js phase remove "${target}" --force)
```
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/arc:progress` — see updated roadmap status
