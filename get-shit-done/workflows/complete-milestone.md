...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/get-shit-done/bin/arc-tools.js summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ~/.claude/get-shit-done/bin/arc-tools.js milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/arc:new-milestone)
