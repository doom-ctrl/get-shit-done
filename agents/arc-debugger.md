---
name: arc-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /arc:debug orchestrator.
...
- `/arc:debug` command (interactive debugging)
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /arc:debug to resume" if context filling up.
</step>
...
```bash
node ~/.claude/get-shit-done/bin/arc-tools.js commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
